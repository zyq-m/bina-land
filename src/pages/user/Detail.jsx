...
const Detail = () => {
	...
	useEffect(() => {
		api.get(`/property/${id}`)
			.then((res) => {
				const dataRes = res.data.property;
				setProperty(dataRes);

				api.get("/property/search", {
					params: { state: dataRes.state },
				}).then((similarRes) => {
					setSimilar(similarRes.data.property);
				});
			})
			.catch((err) => {
				console.log(err);
			...
			<div className="max-w-screen-lg mx-auto pb-12">
				<div className="grid gap-8 grid-cols-2 mb-8 pt-8">
					<div className="col-span-2 relative">
						...
						>
							View all {property?.image?.length} pictures
						</button>
					</div>

					<div className="col-span-2">
						<div className="text-3xl text-black font-bold">
							{money.format(property?.price)} - {property?.sqft}{" "}
							square feet
						</div>
